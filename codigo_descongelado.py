...
class Peça:
    ...
    # Correção mínima: não chamar pyg.event.get() aqui, receber eventos por parâmetro e retornar jogador_atual

    def processa_turno(posicoes_superior, posicoes_inferior, jogador_atual, TILE_SIZE, eventos, pecas):
        """
        Processa o turno atual: seleção, movimentação e alternância de jogador.
        Atualiza todas as peças usando os eventos recebidos.
        Retorna o jogador atual atualizado.
        """
        turno_alternado = False  # Indica se o turno foi completado nesta chamada

        for peca in pecas:
            # Atualiza a peça com a lista global de eventos
            ...
            if peca.selecionada:
                ...
                if permitido:
                    ...
                    peca.representar_selec(movimentos_possiveis, janela, TILE_SIZE)
                else:
                    # Bloqueia peças do outro jogador
                    peca.selecionada = False

            # Corrige movimento e desenha a peça
            peca.checar_movimento(casas_pretas)
            peca.desenhar()

            # Se a peça do jogador atual completou movimento, sinaliza para alternar turno
            if peca.selecionada and not turno_alternado:
                jogador_atual = "inferior" if jogador_atual == "superior" else "superior"
                peca.selecionada = False
                turno_alternado = True
...
while running:
    ...
    # passar os eventos capturados ao chamar (correção mínima)
    jogador_atual = peca.processa_turno(posicoes_superior, posicoes_inferior, jogador_atual, TILE_SIZE, eventos)
