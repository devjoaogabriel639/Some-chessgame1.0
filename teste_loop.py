...
posicaum = -1  # Começa antes do primeiro índice
quantidade_pecas = len(pecas)

# Desenhar tabuleiro e posicionar peças (fora do loop principal)
for _ in range(quantidade_pecas):
    posicaum += 1
    desenha_mapa(tabuleiro)
    janela.blit(superficie, casas_pretas[posicaum])
    pyg.display.flip()
